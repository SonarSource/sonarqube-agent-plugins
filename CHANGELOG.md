...
### Added
- `/sonarqube:configuring-sonarqube` skill — guided setup wizard that installs `sonarqube-cli`, authenticates via `sonar auth login` (credentials stored in system keychain), installs the secrets scanning binary, and runs `sonar integrate claude` to register secrets hooks
