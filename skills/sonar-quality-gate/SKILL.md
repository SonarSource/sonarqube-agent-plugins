...
- If still not found, **omit `projectKey`** in MCP calls and rely on the integration default. The CLI fallback in Step 3 auto-detects the project the same way, so a missing key doesn't block it either — pass `-p` only when targeting a project other than the current one.
...
**If `mcp__sonarqube__get_project_quality_gate_status` is unavailable, fall back to the CLI.** Pass `-p <project-key>` only if you resolved one in Step 1 — otherwise omit it and let the CLI auto-detect the project from `sonar-project.properties`/git; if that also fails to resolve a project, ask the user or invoke sonar-list-projects, then stop.

```bash
sonar quality-gate status [-p <project-key>] [--branch <name> | --pull-request <id>] --format json
```

`--branch` and `--pull-request` are mutually exclusive, same as `--branch`/`--pr` above. Use `--format json` when you need to parse the result; omit it (default `table`) when relaying the output straight to the user. This single call already returns the same `status`/`conditions` shape as above **plus** a `breakdown` on every failing condition — see Step 5, don't fetch it separately.
...
If you need to focus on just one category (for example the user asks specifically "why did coverage fail?"), re-run the CLI with `--category <coverage|duplications|issues|dependency-risks>` (values match the metric groups above; support depends on your CLI version). Leaving `--category` off, as in Step 3, already returns breakdowns for every category at once — only narrow it down on request. `--top <n>` controls how many entries each breakdown includes if you need more than the default.
